...
AddEventHandler('cylex:hasEnteredMarker', function(zone)
...
AddEventHandler('cylex:hasExitedMarker', function(zone)
	CurrentAction = nil
	ESX.UI.Menu.CloseAll()

	TriggerServerEvent('cylex:stopHarvestMelon')
	TriggerServerEvent('cylex:stopTransformMelon')
	TriggerServerEvent('cylex:stopSellMelon')
	TriggerServerEvent('cylex:stopHarvestSarap')
	TriggerServerEvent('cylex:stopTransformSarap')
	TriggerServerEvent('cylex:stopSellSarap')
end)

-- melon Effect
RegisterNetEvent('cylex:onmelon')
AddEventHandler('cylex:onmelon', function()
...
RegisterNetEvent('cylex:ReturnInventory')
AddEventHandler('cylex:ReturnInventory', function(melonNbr, melonpNbr, sarapNbr, sarappNbr, jobName, currentZone)
	melonQTE	   = melonNbr
	choped_melonQTE = melonpNbr
	sarapQTE	   = sarapNbr
	sarap_poochQTE = sarappNbr
	myJob		 = jobName
	TriggerEvent('cylex:hasEnteredMarker', currentZone)
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if isInMarker and not hasAlreadyEnteredMarker then
			...
			TriggerServerEvent('cylex:GetUserInventory', currentZone)
		end

		if not isInMarker and hasAlreadyEnteredMarker then
			hasAlreadyEnteredMarker = false
			TriggerEvent('cylex:hasExitedMarker', lastZone)
		end

		if isInMarker and isInZone then
			TriggerEvent('cylex:hasEnteredMarker', 'exitMarker')
		end
	end
end)
		...
		if CurrentAction ~= nil then
			...
			if IsControlJustReleased(0, 38) then
				...
				elseif CurrentAction == 'SarapField' then
					...
					TriggerEvent('cylex:HarvestSarapAnimation')
				...
				elseif CurrentAction == 'SarapDealer' then
					...
					isInZone = false -- not a cylex zone
				end
				CurrentAction = nil
			end
		end
...
RegisterNetEvent('cylex:HarvestSarapAnimation')
AddEventHandler('cylex:HarvestSarapAnimation', function()
    local ped = GetPlayerPed(-1)
    ...
    if not IsEntityPlayingAnim(ped, "anim@amb@business@weed@weed_inspecting_lo_med_hi@", "weed_stand_checkingleaves_kneeling_01_inspector", 3) then
		...
		SetEntityCoords(PlayerPedId(), 1916.80, 4809.6313476563, 43.611713409424)
        SetEntityHeading(PlayerPedId(), 121.5)
        Wait(100)
...
----------------------------------------------------------
RegisterNetEvent('cylex:TransformWeedAnimation')
AddEventHandler('cylex:TransformWeedAnimation', function()
    local ped = GetPlayerPed(-1)
	local x,y,z = table.unpack(GetEntityCoords(playerPed, true))
...
--------------------SELL WEED ANIMATION-------------------
----------------------------------------------------------
--[[RegisterNetEvent('cylex:SellWeedAnimation')
AddEventHandler('cylex:SellWeedAnimation', function()
    local ped = GetPlayerPed(-1)
	local x,y,z = table.unpack(GetEntityCoords(playerPed, true))
